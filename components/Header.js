...
export default function Header({ onBack, githubStars, onHowItWorks }) {
  return (
    <header className="flex items-center justify-between gap-4 px-5 md:px-8 py-3.5 border-b border-hairline">
      <div className="flex items-center gap-4 min-w-0">
        ...
        <nav className="hidden xl:flex items-center gap-4 ml-2" aria-label="Main navigation">
          {NAV_LINKS.map(([label, href]) => <a key={href} href={href} className="text-xs text-[#8f96a5] hover:text-[#e7e9ee] transition">{label}</a>)}
        </nav>
      </div>
      <div className="flex items-center gap-3 md:gap-4 shrink-0">
        <nav className="hidden lg:flex xl:hidden items-center gap-3" aria-label="Main navigation">
          {NAV_LINKS.map(([label, href]) => <a key={href} href={href} className="text-[11px] text-[#8f96a5] hover:text-[#e7e9ee] transition">{label}</a>)}
        </nav>
